<?php
class Model_Product extends Model_Core_Table
{
    public function __construct()
    {
        parent::__construct();
        $this->setResourceClass('Model_Product_Resource');
        $this->setCollectionClass('Model_Product_Collection');
    }
    public function getStatus()
    {
        if ($this->status) {
            return $this->status;
        }
        return Model_Product_Resource::STATUS_DEFAULT;
    }

    public function getStatusText()
    {
        $statuses = $this->getResource()->getStatusOptions();
        if (array_key_exists($this->status, $this->getResource()->getStatusOptions())) {
            return $statuses[$this->status];
        }
        return $statuses[Model_Product_Resource::STATUS_DEFAULT];
    }

    public function getBase()
    {
        if($this->base_id){
            return Ccc::getModel('product_media')->load($this->base_id);
        }
        return false;
    }

    public function getSmall()
    {
        if($this->small_id){
            return Ccc::getModel('product_media')->load($this->small_id);
        }
        return false;
    }

    public function getThumb()
    {
        if($this->thumb_id){
            return Ccc::getModel('product_media')->load($this->thumb_id);
        }
        return false;
    }

    public function getAttributes()
    {
        $sql = "SELECT * FROM `eav_attribute` WHERE `entity_type_id` = 1 AND `status` = 1";
        $attributes = Ccc::getModel("eav_attribute")->fetchAll($sql);
        return $attributes;
    }

    public function getAttributeValue(Model_Eav_Attribute $attribute)
    {
        $sql = "SELECT `value` FROM `product_{$attribute->backend_type}` WHERE `entity_id` = '{$this->getId()}' AND `attribute_id` = '{$attribute->getId()}'";
        return $this->getResource()->getAdapter()->fetchOne($sql);
    }

}

?>